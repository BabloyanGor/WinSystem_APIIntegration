...
public class IqSoft_API_1_GetUrl_Negative_Test extends BaseTest {
    ...
    public void getUrlAPIValidateResponseWithInvalidToken() throws UnirestException, IOException {
        ...
        HttpResponse<String> response = getUrlAPI(partnerID,productID,clientId,"InvalidToken12345678");
        Unirest.shutdown();
        ...
        softAssert.assertNotEquals(iqSoft01ApiVariables_getProductUrl_response.getResponseCode(),0);
        softAssert.assertNotEquals(iqSoft01ApiVariables_getProductUrl_response.getDescription(),null, "Error Description: " + iqSoft01ApiVariables_getProductUrl_response.getDescription());
...
//        softAssert.assertEquals(iqSoft01ApiVariables_getProductUrl_response.getResponseObject(),null);
    ...
    @Test(priority = 4)
    @Description("Verify getURL API_s response with invalid PartnerId")
    @Severity(SeverityLevel.NORMAL)
    public void getUrlAPIValidateResponseWithInvalidPartnerId() throws UnirestException, IOException {

        SoftAssert softAssert = new SoftAssert();

        HttpResponse<String> response = getUrlAPI(1234,productID,clientId,capturedToken);
        Unirest.shutdown();
        statusCod = response.getStatus();
        jsonObjectBody = new JSONObject(response.getBody());
